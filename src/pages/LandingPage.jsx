...
// Mock Posters Data
const posters = [
  'https://images.unsplash.com/photo-1541963463532-d68292c34b19', // Mock Event 1
  'https://images.unsplash.com/photo-1501281668745-f7f57925c3b4', // Mock Event 2
  'https://images.unsplash.com/photo-1492684223066-81342ee5ff30', // Mock Event 3
  'https://www.robertwalters.co.uk/content/dam/robert-walters-redesign/global/images/resource-solutions/935x530/content-images/RS_RW-C405-smiling%20colleagues%20shake%20hands-935x530px.jpg', // Mock Event 4
]

const PosterRow = ({ direction = 'left' }) => (
  <div className="flex w-[200%] gap-4 py-2 opacity-15">
    <div className={`flex shrink-0 gap-4 ${direction === 'left' ? 'animate-scroll-left' : 'animate-scroll-right'}`}>
      {[...posters, ...posters, ...posters].map((src, i) => (
        <div key={i} className="h-64 w-44 overflow-hidden rounded-2xl bg-white/10">
          <img src={src} className="h-full w-full object-cover grayscale" alt="event" />
        </div>
      ))}
    </div>
  </div>
...
export default function LandingPage() {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden text-white bg-[#0A0B10]">
      {/* BACKGROUND LAYER */}
      <div className="absolute inset-0 z-0 overflow-hidden bg-[#0A0B10]">
        <Aurora colorStops={['#FEADC8', '#5545CE', '#4E4094']} amplitude={1.1} blend={0.62} speed={0.9} />
      </div>

      {/* FLOATING POSTERS LAYER */}
      <div className="absolute inset-0 z-0 pointer-events-none flex flex-col justify-center gap-4 rotate-[-12deg] scale-125">
        <PosterRow direction="left" />
        <PosterRow direction="right" />
        <PosterRow direction="left" />
      </div>
      ...
      <nav className="absolute left-4 right-4 top-4 z-20 flex items-center justify-between font-medium md:left-8 md:right-8">
        ...
        >
          <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/18 font-bold shadow-inner">M</span>
          <span className="text-sm font-semibold tracking-wide md:text-base">MentorLink</span>
        </button>

        <div className="flex items-center ">
          
          <button type="button" onClick={() => navigate('/login')} className="rounded-xl border border-white/25 bg-white/18 px-4 py-2 text-sm transition hover:bg-white hover:text-[#1a1b20]">Login</button>
        </div>
      </nav>
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 md:gap-4">
          <section className="relative overflow-hidden rounded-[28px] border border-white/18 bg-white/[0.10] px-5 py-6 text-center shadow-[0_24px_90px_rgba(40,20,120,0.28)] backdrop-blur-2xl md:px-10 md:py-8 lg:px-14 lg:py-9">
            ...
            <div className="relative z-10">
              ...
              <div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row md:mt-6">
                <button type="button" onClick={() => navigate('/login')} className="rounded-xl bg-white px-6 py-3 text-sm font-bold text-[#5545CE] shadow-lg transition hover:scale-[1.02] hover:bg-white/92 md:text-base">Get Started</button>
              ...
              <div className="mt-5 flex flex-wrap justify-center gap-2 text-xs text-white/90 md:text-sm">
                {['Explore Communities', 'Track Applications', 'Register for Events', 'Find Student Matches'].map(item => (
                  <span key={item} className="rounded-full border border-white/18 bg-white/10 px-3 py-1.5 backdrop-blur-md">{item}</span>
                ))}
              </div>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {miniCards.map(([label, text]) => (
              <GlassBlobCard key={label} className="border-white/18 bg-white/[0.10] px-5 py-3.5 text-white shadow-[0_12px_36px_rgba(40,20,120,0.18)] md:py-4">
                <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-white/60">{label}</p>
                <p className="mt-1.5 text-sm font-semibold leading-snug md:text-base">{text}</p>
              </GlassBlobCard>
            ))}
          </section>
        </div>
